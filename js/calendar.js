...
export async function initCalendar() {
  ...
  try {
    const [rawLog, weather, cMap] = await Promise.all([
      getWearLog(14, 14),
      getWeatherForecast(),
      getColourMap(),
    ]);
...
function makeDayCard(date, dStr, todayStr) {
  ...
  if (worn.length) {
    ...
    worn.forEach(row => {
      const hex = resolveHex(row.items?.color);
    ...
    worn.forEach(row => {
      ...
      html += `
        <div class="outfit-item-row" data-log-id="${row.id}">
          <div class="swatch swatch-lg" style="background:${hex}"></div>
          <div class="outfit-item-info">
            <div class="outfit-item-name">${esc(item.name ?? 'Unknown item')}</div>
            <div class="outfit-item-meta">${esc(item.brand ?? '')} · ${esc(item.color ?? '')}</div>
          </div>
